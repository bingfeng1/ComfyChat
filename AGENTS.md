...
- **Generations feature** (`/generations` page): `WorkflowGenerationConfig` is 1:1 with workflow (`UNIQUE(workflow_id)`); stores `api_template` (executable API-format JSON) + `fields_json` (parameter field definitions). `Generation` rows reference a workflow_id and a `prompt_id` from ComfyUI. POST `/generations` triggers FastAPI `BackgroundTasks` polling every 2 s (fresh session per iteration via `services["database"]`) and downloads outputs via `get_image` to `storage/outputs/{YYYY-MM}/` (flat, no per-gen folder). Filenames dedupe via `_dedup_target` (collision → `_1/_2` suffix). Deleting/cancelling a generation uses `_delete_outputs` to remove only its `outputs_json` files — NEVER `rmtree` the month dir (would delete other generations' images). Image serving has a path-traversal guard (`Path(filename).name` + `resolve()` + parent-equality).
