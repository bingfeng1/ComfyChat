...
- Backend tests: `backend\.venv\Scripts\python -m pytest backend/tests/<file> -v`. Target: 253 tests collected, 252 pass + 1 known Windows fail (`test_check_database_returns_false_when_path_unwritable` — see Quirks).
...
- **Workspaces** (`/workspaces`): user-defined labels for organizing generations.
  ...
  - Routes: `GET/POST /workspaces`, `PATCH /workspaces/{id}` (rename), `DELETE /workspaces/{id}`, `GET /workspaces/{id}/generation-count`, `POST /workspaces/{id}/assign/{generation_id}` (single-add, idempotent). Generation endpoints accept `workspace_ids` (create + replace) and expose `DELETE /generations/{id}/workspaces/{ws_id}` for single removal.
  - Frontend: `WorkspacesView.vue` (CRUD), `useWorkspaces` composable (shared list cache), workspace multi-select + inline "+" create modal in `GenerationCreateModal.vue`, chip column with add/remove in `GenerationsView.vue`.
