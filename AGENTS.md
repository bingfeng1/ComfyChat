...
- `storage/` — runtime files (SQLite, uploads, outputs, thumbs, tmp). Fully gitignored; never commit, do not assume empty. Generated images live flat under `storage/outputs/{YYYY-MM}/` (no per-generation subfolder).
...
- Backend tests: `backend\.venv\Scripts\python -m pytest backend/tests/<file> -v`. Target: 267 tests collected, 266 pass + 1 known Windows fail (`test_check_database_returns_false_when_path_unwritable` — see Quirks).
...
- **Global NSFW switch** lives in a Pinia store (`frontend/src/stores/nsfw.ts`). `useNsfwFilter()` composable wraps it via `storeToRefs` — old call sites (`TopBar`, `LoraArrayField`, `GenerationCreateModal`, `useGenerations`, `LorasView`) keep working unchanged. LocalStorage (`cc_nsfw_enabled`) is the persistence layer for reload-state; the store hydrates from it on construction.
- **Sidebar nav is driven by `router.ts` meta.** Each route declares `meta: { label, icon, sidebar? }`. `getNavItems()` in `router.ts` returns the filtered list (drop routes with `meta.sidebar === false`); `Sidebar.vue` consumes it. Adding a new menu item only requires editing `router.ts` — no Sidebar edits. Don't hardcode route paths in `Sidebar.vue`.
- **Workspaces** (`/workspaces`): user-defined labels for organizing generations.
  - Model: `workspaces(id, name UNIQUE, created_at, updated_at)` + `generation_workspace_links(generation_id, workspace_id)` (composite PK, FK CASCADE both sides).
  - **Many-to-many** between generations and workspaces — one generation can be in N workspaces, one workspace contains M generations.
  - **No NSFW on workspace.** NSFW is per-generation (driven by LoRA `is_nsfw`); when global NSFW is off, NSFW generations are hidden, but workspaces themselves remain visible (workspaces are neutral labels).
  - **Delete semantics:** removing a workspace deletes only the `workspaces` row + its links. Generation rows, image files, and other workspaces are untouched — like a favorites folder.
  - **API surface** returns `generation_count` + `preview[]` (most-recent N thumbnails by `created_at desc`) on `WorkspaceOut` — list/detail/create/rename all include them so the UI can show previews without N+1. `preview_for_workspace(ws_id, limit)` lives in `WorkspaceRepository`.
  - Routes: `GET/POST /workspaces`, `GET /workspaces/{id}` (detail), `PATCH /workspaces/{id}` (rename), `DELETE /workspaces/{id}`, `GET /workspaces/{id}/generation-count`, `POST /workspaces/{id}/assign/{generation_id}` (single-add, idempotent). Generation endpoints accept `workspace_ids` (create + replace) and expose `DELETE /generations/{id}/workspaces/{ws_id}` for single removal.
  - Frontend: `WorkspacesView.vue` is a **masonry card grid** (CSS multi-column layout) — each card shows 2×2 preview thumbnails + name + count + relative updated time. Click anywhere on the card → `/workspaces/:id`. Hover-only action buttons (rename/delete) use `stopPropagation`. `WorkspaceDetailView.vue` (`/workspaces/:id`, sidebar hidden) is a masonry tile grid of generations in that workspace; click tile → existing `GenerationDetailModal`. `GenerationCreateModal` accepts a `preselectWorkspaceId` prop for "create in this workspace" flow.
- **Generations feature** (`/generations` page): `WorkflowGenerationConfig` is 1:1 with workflow (`UNIQUE(workflow_id)`); stores `api_template` (executable API-format JSON) + `fields_json` (parameter field definitions). `Generation` rows reference a workflow_id and a `prompt_id` from ComfyUI. POST `/generations` triggers FastAPI `BackgroundTasks` running `GenerationService._watch_and_download(gen_id)`, which calls `ComfyUIClient.wait_for_history(prompt_id)` — a **WebSocket-driven wait** on `/ws?clientId=<uuid>` for `executing{node:null}` or `execution_error` events (filtered by `prompt_id`). No polling, no arbitrary timeouts: completion is signalled by ComfyUI itself, with `/history` as a fast-path pre-check + WS-failure fallback. Images downloaded via `get_image` to `storage/outputs/{YYYY-MM}/` (flat, no per-gen folder). Filenames dedupe via `_dedup_target` (collision → `_1/_2` suffix). Deleting/cancelling a generation uses `_delete_outputs` to remove only its `outputs_json` files — NEVER `rmtree` the month dir (would delete other generations' images). Image serving has a path-traversal guard (`Path(filename).name` + `resolve()` + parent-equality). `Generation.poll_miss_count` column is **legacy** from the old polling code — left in place for back-compat, no longer used by `_watch_and_download`.
